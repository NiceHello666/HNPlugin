...
public class Tpa implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (!requester.hasPermission("hnplugin.command.tpa")) {
        ...
        if (strings.length != 1) {
            ...
            usageMsg.replaceAll(line -> line.replace("%prefix%", lang.getStringList("Prefix").get(0)).replace("%cmd%", "tpa"));
        ...
        TPAManager.sendRequest(requester, target, TPAManager.TPARequestType.TPA, expireTime);

        List<String> sentMsg = new ArrayList<>(lang.getStringList("TpaRequestSent"));
        ...
        util.sendClickableTPARequest(target, lang.getStringList("Prefix").get(0), requester.getName(), lang.getString("TpaRequestReceived"), lang.getString("TpaAcceptButton"), lang.getString("TpaDenyButton"), "/tpaccept " + requester.getName(), "/tpdeny " + requester.getName());
        return true;
    }
}
